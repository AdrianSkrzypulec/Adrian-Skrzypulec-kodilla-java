...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        long tastQuantity = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .count();

        long taskTime = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(task -> Period.between(task.getCreated(), LocalDate.now()).getDays())
                .reduce(0, (sum, current) -> sum = sum + current );

        long avarageTime = taskTime / tastQuantity;

        //Then
        assertEquals(10, avarageTime);
    }
